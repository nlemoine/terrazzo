...
export interface BundleOptions {
  ...
  parse?: (src: any, filename: URL) => DocumentNode;
...
export async function bundle(
...
): Promise<{ document: DocumentNode; sources: Record<string, InputSourceWithDocument>; refMap: RefMap }> {
  const cache: Record<string, InputSourceWithDocument> = {};
  const refMap: RefMap = {};
  const parse: NonNullable<BundleOptions['parse']> = (src, filename) => {
    if (typeof src === 'string' && !maybeRawJSON(src)) {
      if (yamlToMomoa) {
        const result = yamlToMomoa(src);
        return userParse ? userParse(print(result, { indent: 2 }), filename) : result;
      }
    ...
    return userParse
      ...
      : momoaParse(typeof src === 'string' ? src : JSON.stringify(src, undefined, 2), {
          mode: 'jsonc',
          ranges: true,
          tokens: true,
        });
  ...
  const resolvedDocs = await Promise.all<DocumentNode>(
    sources.map(async (source) => {
      const resolved: InputSourceWithDocument = { ...source, document: parse(source.src, source.filename) };
      ...
      if (typeof resolved.src !== 'string') {
        resolved.src = print(resolved.document, { indent: 2 });
      }
...
export async function resolveRef(
  ...
  if (!sources[filename.href]) {
    ...
    sources[filename.href] = { filename, src: rawSource, document: parse(rawSource, filename) };
  }
