...
import type { InputSource, ModeRefMap, ParseOptions } from '../types.js';
...
export async function loadSources(
  ...
  const modeRefMap = {} as ModeRefMap;

  try {
    const result = await bundle(sources, {
      parse: transform?.root ? (src) => transform!.root!(src, '.', toMomoa(src)) : undefined,
      yamlToMomoa,
    });
    document = result.document;
    sourceByFilename = result.sources;
    modeRefMap['.'] = result.refMap;
  ...
  resolveAliases(tokens, { logger, sources: sourceByFilename, modeRefMap });
...
function toMomoa(srcRaw: any): DocumentNode {
  return momoaParse(typeof srcRaw === 'string' ? srcRaw : JSON.stringify(srcRaw, undefined, 2), {
    mode: 'jsonc',
    ranges: true,
    tokens: true,
  });
